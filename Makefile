...
WEB_PORT := $(shell grep WEB_PORT .env | sed "s/WEB_PORT=//")
...
run: .env# コンテナをバックグラウンドで起動
	docker-compose up -d
	@echo "\nAccess to http://localhost:$(WEB_PORT)\nIf you want to connect to develop environment from external host, link http://$(IP):$(WEB_PORT)"
...
.PHONY: ip rebuild refresh restart